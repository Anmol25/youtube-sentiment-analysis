...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_KEY = 'YOUTUBE-API';  // Replace with your actual YouTube Data API key
        ...
        <div class="section">
          ...
          <ul class="comment-list">
            ${predictions.slice(0, 25).map((item, index) => {
              ...
              const sentimentLabel = item.sentiment === -1 ? 'Negative' :
                                    item.sentiment === 0 ? 'Neutral' :
                                    item.sentiment === 1 ? 'Positive' : 'Unknown';

              return `
                <li class="comment-item">
                  <span>${index + 1}. ${item.comment}</span><br>
                  <span class="comment-sentiment">Sentiment: ${sentimentLabel}</span>
                </li>`;
            }).join('')}
          </ul>
        </div>`;
